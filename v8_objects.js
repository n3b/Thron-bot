...
Strategy = function(b)
{
    ...
    var floodfill = function(node)
    {
        ...
        node.each(function()
        {
            val = 40 - 4 * b.map.isPossibleCutVertex(this) - 2 * this.edges;
    ...
    var deepIteration = function(node, level)
    {
        ...
        for( var i = 0; i < 4; i++ )
        {
            ...
            val = 1 + deepIteration(next, level - 1);
            spacesForDeepIteration++;
            b.me.unmove();
    ...
    this.fill = function()
    {
        ...
        b.me.node().unlock();
        spacesForDeepIteration = b.components.spaces(b.me.node()) - 1;
        b.me.node().lock();

        for( var i = 1; i <= spacesForDeepIteration; i++ )
        {
            if( isTimedOut() ) break;
    ...
    this.attack = function()
    {
        ...
        node.each(function(d)
        {
            ...
            {
                b.voronoi.calculate();
                worst = b.voronoi.available(b.me) - b.voronoi.available(target) - 100 * target.distance - 100 * this.edges;
            }
            else
            {
