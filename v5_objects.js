...
Strategy = function(b)
{
    ...
    {
        timer.start('fill');
        var result = deepfill(b.me.node(), 30, b.me.node());
...
//                    var available = myspace - enemyspace - 4 * target.distance;
                    var available = myspace - enemyspace - 100 * target.distance;
                    if( myspace / (enemyspace + 1) < 0.1 ) available = -1000000;
                    target.unmove();
                    if( available < worst ) worst = available;
                    if( enemyspace / (myspace + 1) < 0.1 ) worst = 1000000;
                });
            }
            ...
            node.each(function(d)
            {
                if( b.components.component(this) === b.components.component(target.node()) )
                {
                    ...
                    target.node().each(function()
                    {
                        target.move(this);
                        var count = 0;
                        b.me.node().each(function(){count++});
                        if( 0 === count ) val = -1000000;
                        target.unmove();
                    });
